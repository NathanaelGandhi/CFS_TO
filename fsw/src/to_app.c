...
int32 TO_AppInit(void)
{
    ...
    {
        iStatus = TO_ERROR;
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "TO - Custom Init failed.");
        goto TO_AppInit_Exit_Tag;
    }
    ...
    {
        iStatus = TO_ERROR;
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "TO - Table Init failed.");
        goto TO_AppInit_Exit_Tag;
    }
    ...
    {
        iStatus = TO_ERROR;
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "TO - Pipe Init failed.");
        goto TO_AppInit_Exit_Tag;
    }

    /* Install the cleanup callback */
    OS_TaskInstallDeleteHandler(&TO_CleanupCallback);

TO_AppInit_Exit_Tag:
    if (iStatus == CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(TO_INIT_INF_EID, CFE_EVS_EventType_INFORMATION, "Application initialized");
    }
    else
    {
...
int32 TO_InitEvent(void)
{
    ...
    CFE_PSP_MemSet((void *)g_TO_AppData.EventTbl, 0x00, sizeof(g_TO_AppData.EventTbl));
    ...
    iStatus = CFE_EVS_Register(g_TO_AppData.EventTbl, TO_EVT_CNT, CFE_EVS_EventFilter_BINARY);
    if (iStatus != CFE_SUCCESS)
    {
...
int32 TO_InitData(void)
{
    ...
    /* Init output data */
    CFE_PSP_MemSet((void *)&g_TO_AppData.OutData, 0x00, sizeof(g_TO_AppData.OutData));
    CFE_MSG_Init(&g_TO_AppData.OutData.TlmHeader.Msg, TO_OUT_DATA_MID, sizeof(g_TO_AppData.OutData));

    /* Init housekeeping packet */
    CFE_PSP_MemSet((void *)&g_TO_AppData.HkTlm, 0x00, sizeof(g_TO_AppData.HkTlm));
    CFE_MSG_Init(&g_TO_AppData.HkTlm.TlmHeader.Msg, TO_HK_TLM_MID, sizeof(g_TO_AppData.HkTlm));
    ...
    for (ii = 0; ii < TO_MAX_NUM_ROUTES; ++ii)
    {
        ...
        CFE_PSP_MemSet((void *)pTlmPipe->cTlmPipeName, '\0', sizeof(pTlmPipe->cTlmPipeName));
    ...
    for (ii = 0; ii < TO_NUM_CF_CHANNELS; ++ii)
    {
        ...
        strncpy(pCfChnl->cCfCntSemName, TO_CF_THROTTLE_SEM_NAME, OS_MAX_API_NAME);
    }
...
int32 TO_InitTable(void)
{
    ...
    iStatus = CFE_TBL_Register(&g_TO_AppData.tableHandle, TO_CONFIG_TABLENAME, sizeof(TO_ConfigTable_t),
                               CFE_TBL_OPT_DEFAULT, (CFE_TBL_CallbackFuncPtr_t)TO_ValidateTable);

    if (iStatus != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CFE_TBL_Register() returned error 0x%08x.  Aborting table init.", iStatus);

        goto end_of_function;
    }

    /* Load the TO configuration table file */
    iStatus = CFE_TBL_Load(g_TO_AppData.tableHandle, CFE_TBL_SRC_FILE, TO_CONFIG_FILENAME);

    if (iStatus != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CFE_TBL_Load() returned error 0x%08x.  Aborting table init.", iStatus);
    ...
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CFE_TBL_Manage() returned error 0x%08x.  Aborting table init.", iStatus);
    ...
    iStatus = CFE_TBL_NotifyByMessage(g_TO_AppData.tableHandle, TO_APP_CMD_MID, TO_MANAGE_TABLE_CC, 0);
    if (iStatus != CFE_SUCCESS)
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
...
int32 TO_InitPipe(void)
{
    ...
    CFE_PSP_MemSet((void *)g_TO_AppData.cSchPipeName, '\0', sizeof(g_TO_AppData.cSchPipeName));
    strncpy(g_TO_AppData.cSchPipeName, "TO_SCH_PIPE", OS_MAX_API_NAME - 1);

    /* Subscribe to Wakeup messages */
    iStatus = CFE_SB_CreatePipe(&g_TO_AppData.SchPipeId, g_TO_AppData.usSchPipeDepth, g_TO_AppData.cSchPipeName);
    if (iStatus == CFE_SUCCESS)
    {
    ...
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "Failed to create SCH pipe (Error:0x%08x)",
                          iStatus);
    ...
    CFE_PSP_MemSet((void *)g_TO_AppData.cCmdPipeName, '\0', sizeof(g_TO_AppData.cCmdPipeName));
    strncpy(g_TO_AppData.cCmdPipeName, "TO_CMD_PIPE", OS_MAX_API_NAME - 1);

    /* Subscribe to command messages */
    iStatus = CFE_SB_CreatePipe(&g_TO_AppData.CmdPipeId, g_TO_AppData.usCmdPipeDepth, g_TO_AppData.cCmdPipeName);
    if (iStatus == CFE_SUCCESS)
    {
    ...
    {
        CFE_EVS_SendEvent(TO_INIT_ERR_EID, CFE_EVS_EventType_ERROR, "Failed to create CMD pipe (Error:0x%08x)",
                          iStatus);
        goto TO_InitPipe_Exit_Tag;
    }
    ...
    for (jj = 0; jj < TO_MAX_NUM_ROUTES; ++jj)
    {
        ...
        if (pRoute->usExists)
        {
            ...
            iStatus  = CFE_SB_CreatePipe(&pTlmPipe->cfePipeId, pTlmPipe->usTlmPipeDepth, pTlmPipe->cTlmPipeName);
            ...
            if (pRoute->sCfChnlIdx >= 0)
            {
                 ...
                 * CF channel. */
                iStatus =
                    OS_CountSemCreate(&pCfChnl->uiCfCntSemId, pCfChnl->cCfCntSemName, pTlmPipe->usTlmPipeDepth, 0);
...
int32 TO_ValidateTable(void *table)
{
    ...
    for (ii = 0; ii < TO_MAX_TBL_ENTRIES; ii++)
    {
        ...
        if (entry->usMsgId != TO_UNUSED_ENTRY && entry->usMsgId != TO_REMOVED_ENTRY)
        {
            ...
            if (reachedUnused)
            {
                CFE_EVS_SendEvent(TO_TBL_ERR_EID, CFE_EVS_EventType_ERROR,
                                  ...
                                  "Unused Entry before entry index:%d",
                                  ii);
                iStatus = TO_ERROR;
                goto end_of_function;
            }
            ...
            for (jj = ii + 1; jj < TO_MAX_TBL_ENTRIES; jj++)
            {
                ...
                else if (entryCmp->usMsgId != TO_REMOVED_ENTRY && entry->usMsgId == entryCmp->usMsgId)
                {
                    CFE_EVS_SendEvent(TO_TBL_ERR_EID, CFE_EVS_EventType_ERROR,
                                      "Table Validation failed. "
                                      "Duplicate MID:0x%04x",
                                      entry->usMsgId);
...
int32 TO_RcvMsg(int32 iBlocking)
{
    ...
    if (iStatus == CFE_SUCCESS)
    {
        ...
        switch (MsgId)
        {
            ...
            default:
                CFE_EVS_SendEvent(TO_MSGID_ERR_EID, CFE_EVS_EventType_ERROR, "Recvd invalid SCH usMsgId (0x%04X)",
                                  MsgId);
        }
    }
    ...
    {
        CFE_EVS_SendEvent(TO_PIPE_ERR_EID, CFE_EVS_EventType_ERROR, "TO: SB pipe read error (0x%08x), app will exit",
                          iStatus);
        g_TO_AppData.uiRunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
...
void TO_ProcessNewData(TO_TlmPipe_t *pTlmPipe, uint16 usRouteId)
{
    ...
    if (g_TO_AppData.usOutputEnabled && g_TO_AppData.usOutputActive && g_TO_AppData.routes[usRouteId].usIsEnabled)
    {
    ...
    while (bGotNewMsg)
    {
        if (CFE_SB_ReceiveBuffer(&pTlmMsg, pTlmPipe->cfePipeId, CFE_SB_POLL) == CFE_SUCCESS)
        {
            ...
            if (g_TO_AppData.usOutputEnabled && g_TO_AppData.usOutputActive)
            {
                ...
                else
                {
                    ...
                    if (pEntry->usState && (pEntry->usRouteMask & (1 << usRouteId)) &&
...
void TO_ProcessNewCmds(void)
{
    ...
    while (bGotNewMsg)
    {
        if (CFE_SB_ReceiveBuffer(&pCmdMsg, g_TO_AppData.CmdPipeId, CFE_SB_POLL) == CFE_SUCCESS)
        {
            ...
            switch (usMsgId)
            {
                ...
                default:
                    ...
                    CFE_EVS_SendEvent(TO_MSGID_ERR_EID, CFE_EVS_EventType_ERROR, "Recvd invalid CMD usMsgId (0x%04X)",
                                      usMsgId);
                    break;
            }
        }
    ...
    if (pMsg != NULL)
    {
        ...
        switch (uiCmdCode)
        {
            ...
            default:
                ...
                {
                    g_TO_AppData.HkTlm.usCmdErrCnt++;
                    CFE_EVS_SendEvent(TO_CMD_ERR_EID, CFE_EVS_EventType_ERROR, "Recvd invalid cmd code (%d)",
                                      uiCmdCode);
                }
                break;
        }
